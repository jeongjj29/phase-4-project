...
def get_all_item_prices_grouped():
    item_prices = (
        db.session.query(ItemPrice)
        .join(Store)
        .join(Item)
        .order_by(Store.name, ItemPrice.created_at.desc())
        .all()
    )
    ...
    for item_price in item_prices:
        order_key = (item_price.store.id, item_price.created_at.date())
        if order_key not in orders:
            orders[order_key] = {
                ...
                'created_at': item_price.created_at.date().isoformat(),
                'items': []
            }
        
        orders[order_key]['items'].append({
            'id': item_price.id,
            'item_name': item_price.item.name,
