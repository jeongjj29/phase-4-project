...
const AllOrdersPage = () => {
  ...
  const handleItemSort = (orderKey, key) => {
    const currentItemSort = itemSortConfig[orderKey] || { key: '', direction: 'ascending' };
    let direction = 'ascending';
    if (currentItemSort.key === key && currentItemSort.direction === 'ascending') {
      direction = 'descending';
    }
  ...
  const getSortIcon = (key, config) => {
    if (config?.key === key) {
      return config.direction === 'ascending' ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />;
    }
    return null;
  };
      ...
      <table>
        <thead>
          <tr>
            ...
            <th className='order_store_th' onClick={() => handleSort('store.name')} style={{ cursor: 'pointer' }}>
              Store {getSortIcon('store.name', sortConfig)}
            </th>
            <th className='order_date_th' onClick={() => handleSort('created_at')} style={{ cursor: 'pointer' }}>
        ...
        <tbody>
          {orders.map((order, index) => {
            const orderKey = `${order.store.id}_${order.created_at}`;
            const itemSort = itemSortConfig[orderKey] || { key: '', direction: 'ascending' };
            ...
            return (
              <React.Fragment key={index}>
                <tr>
                  ...
                  <td className='store'>
                    <Link to={`/stores/${order.store.id}`}>{order.store.name}</Link>
                  </td>
                  <td>{order.created_at}</td>
                  <td className="visit_order_btn">
                    <Link to={`/order?store=${order.store.name}&date=${order.created_at}`}>
                  ...
                  <tr className='order_purchases_lst'>
                    <td colSpan={4}>
                      <table className="order_purchases_lst">
                        <thead>
                          <tr>
                            <th onClick={() => handleItemSort(orderKey, 'name')} style={{ cursor: 'pointer' }}>
                              Item {getSortIcon('name', itemSort)}
                            </th>
                            <th onClick={() => handleItemSort(orderKey, 'price')} style={{ cursor: 'pointer' }}>
